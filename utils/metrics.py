...
def reconstruction_mse(output, target, **kwargs):
    ...
    full_pixel_mses = (output[:, :kwargs['cfg'].gen_len[kwargs['setting']]] - target[:, :kwargs['cfg'].gen_len[kwargs['setting']]]) ** 2
...
def extrapolation_mse(output, target, **kwargs):
    ...
    full_pixel_mses = (output[:, kwargs['cfg'].gen_len.train:] - target[:, kwargs['cfg'].gen_len.train:]) ** 2
